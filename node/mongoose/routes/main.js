...
app.post('/insert', function (req, res, next) {
    ...
    user.save(function (err, slience) {
        if (err) {
            console.log('err')
            res.status(300).send('Insert Error')
            retrun; 
        }
...
function query3(callback) {
    // select * from users where ciry="Seoul" order by userid limit 3
...
function query6(callback) {
    // using querybuilder
    // select userid, name, age from users city="seoul" and age > 15 and age < 23 order by age
